...
const ProductSpecification = (props) => {
    ...
    const GetSpecification = async () => {
        try {
            const response = await get(endpoints.GetSpecification)
    ...
    const GetProductSpecificationByProductId = async (id) => {
        try {
            const response = await get(endpoints.GetProductSpecificationByProductId + "?pProductId=" + id)
            console.log('datar', response);
            if (response.isSuccess === 200) {
    ...
    useEffect(() => {
        GetProductSpecificationByProductId(productId);
        GetSpecification();
    }, []);
    ...
    const onSave = async (e) => {
        e.preventDefault();
        try {
            const response = await post(endpoints.SaveProductSpecification, formData);
            if (response.isSuccess === 200) {

                toast.success("Product Specification save successfully");
                onReset();
                GetProductSpecificationByProductId(productId);
    ...
    const onDelete = async (id) => {
        ...
        const response = await get(endpoints.DeleteProductSpecificationById + "?pProductSpecificationId=" + id)
        if (response.isSuccess == 200) {
            GetProductSpecificationByProductId(productId);
        }
    }
                    ...
                    <div className="row mt-3 product_hedding_table_main">
                        <table className="table table-bordered">
                            <thead>
                                <tr>
                                    <th>S.No</th>
                                    <th>Specification Display Name</th>
                                ...
                                {specificationData?.length > 0 ? (
                                    specificationData?.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>{item.specificationDisplayName}</td>
                                            ...
                                            <td>
                                                <div className="action_btn_weepr">
                                                    ...
                                                    <span className="delete_icon_weepr" onClick={() => onDelete(item.productSpecificationId)}>
                            ...
                            <form onSubmit={onSave}>
                                ...
                                <div className="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 ">
                                    <div className="send_quote_wepper">
                                        <select name="specificationId" id="specificationId" className="form-control" value={formData.specificationId} onChange={handleChange} required >
                                            <option value="0" className="option">Select Specification</option>
                                            {specificationsData?.map((item, i) => (
                                                <option key={item.specificationId} value={item.specificationId} className="option">{item?.displayName}</option>
                                            ))}
                                        </select>
                                ...
                                <div className="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 ">
                                    <div className="send_quote_wepper">
                                        <input type="text" name="description" id="description" className="form-control" placeholder="Description" required value={formData.description} onChange={handleChange} />
                                    </div>
                                </div>
